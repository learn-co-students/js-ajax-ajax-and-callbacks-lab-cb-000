  ...
  $(document).ready(function () {
    $.get('https://api.github.com/search/repositories?q=' + searchTerms, function(response) {
      ...
      $.each(response.items, function(idx, item) {
        htmlString = htmlString + '<li><a href="' + item.html_url + '">' + item.name + '</a> - Description: ' + item.description + ' - Owner: <a href="https://github.com/' + item.owner.login + '">' + item.owner.login + '<img height="32" width="32" src="' + item.owner.avatar_url + '"></a> - <a href="#" onclick="showCommits(this)" data-repository="' + item.name + '" data-username="' + item.owner.login + '">Show Commits</a></li>';
      });
      htmlString += '</ul>';
